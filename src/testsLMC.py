...
import numpy as np
import time
from phd_tools.LMC.classLMC import LassoMC
...
m[0] = 4.0
m[1] = 1.0
print("True mean is", np.sum(m*0.5))
print("True std is", np.sqrt(1/12*np.dot(m,m)))

def get_test_input(N, random_state = None):
    np.random.seed(random_state)
    return np.random.rand(N,dim)

def get_test_input_and_output(N, random_state = None):
    np.random.seed(random_state)
    X = get_test_input(N, random_state = np.random.randint(low = 1, high = 1000))
    y = np.dot(X,m) + eps * np.random.randn(X.shape[0])  # Linear model with noise.
    return X,y

# print(get_test_input(4,42))
# print(get_test_input_and_output(2,42))
